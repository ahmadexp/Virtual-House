void TestApp::drawhud(){
	...
	// Calculate target position (Moved outside !testmode block to allow checking condition in all modes)
	if(allocentric){
		switch(targetwin){
		case 0:
	...
	if((target_x-target_offset<playerxpos)&&(target_x+target_offset>playerxpos)&&(target_y-target_offset<(playerypos))&&(target_y+target_offset>(playerypos))&&(target_z-target_offset<playerzpos)&&(target_z+target_offset>playerzpos)){
		...
		// Only draw text if NOT in testmode (or consistent with previous behavior)
		...
		if(sound){ 
			if(kidmode){
				PlatformPlaySound("Media/applause.wav", true); sound = 0;
			}
			else{
				switch(lang){
					case 1:   //English
						PlatformPlaySound("Media/goodjob.mp3", true); sound = 0;
						break;
					case 2:   //French
						PlatformPlaySound("Media/bontravail.wav", true); sound = 0;
						break;
					case 3:   //Spanish
						PlatformPlaySound("Media/buentrabajo.wav", true); sound = 0;
						break;
					case 4:   //German
						PlatformPlaySound("Media/gutgemacht.wav", true); sound = 0;
						break;
				}
			}
		}
...
#else
		if(autoexit){usleep(3000000);exit(0);}
#endif
	}
	...
	if(kidmode&&animal_background_sound)
	{
		if(sound_delay)
			...
			switch(targetwin){
				case 0:
					PlatformPlaySound("Media/cat.wav", true); sound = 0;
					break;
				case 1:
					PlatformPlaySound("Media/dog.wav", true); sound = 0;
					break;
				case 2:
					PlatformPlaySound("Media/cow.wav", true); sound = 0;
					break;
			}
		}
	}
