...
static const int maxnormalcount=10000;
static float loadednormal[maxnormalcount][3];

static const int maxtexturecount=10000;
static float loadedtexture[maxtexturecount][2];

static const int maxfacecount=10000;
static int loadedface[maxfacecount][9];
...
float* morph(float* mesh1,float* mesh2,int vertexcount,float progress){
	float *drawarray = new float[vertexcount*3];
	for(int a=0; a<vertexcount*3; a++)
		drawarray[a]=mesh1[a]*(1-progress)+mesh2[a]*(progress);
	return drawarray;
}
