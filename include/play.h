...
#else
		calibjoy=0;  // No-op on non-Windows
#endif
...
// intro
	...
	if(control_buffer[0][latency_read_pointer]){
		if(indoor){
			playerxposmov+=InOutRatio*gamespeed*(0.04*cos(-camxang)*cos(camyang-90*radiansindegree));
			//playeryposmov+=(0.04*sin(-camxang));
			playerzposmov+=InOutRatio*gamespeed*(0.04*cos(-camxang)*sin(camyang-90*radiansindegree));
		}
		else{
	...
	if(usejoystick){											//Joystick control	
		...
		if(joyGetPos(0,&joystick))	usejoystick=0;				//obtain joystick value and check the presence
			...
			if((32768-(float)joystick.wYpos>1500)||(32768-(float)joystick.wYpos<-1500)){
				if(!start_motion){	
					if(indoor){
						playerxposmov+=(32768-(float)joystick.wYpos)*(InOutRatio*gamespeed*0.58*JoyTransVel/32768)*cos(-camxang)*cos(camyang-90*radiansindegree);
						playerzposmov+=(32768-(float)joystick.wYpos)*(InOutRatio*gamespeed*0.58*JoyTransVel/32768)*cos(-camxang)*sin(camyang-90*radiansindegree);
					}
					else{
	...
	if((!start_motion)&&indoor&&((judge_res!=0x07))){
		worldgrid[a_win_pos[0]][a_win_pos[1]][a_win_pos[2]][0]=window_tile; 	
		worldgrid[b_win_pos[0]][b_win_pos[1]][b_win_pos[2]][0]=window_tile; 
		worldgrid[c_win_pos[0]][c_win_pos[1]][c_win_pos[2]][0]=window_tile; 
	}
	else{
